const fs = require('fs');
const file = 'temp_topology_panels.tsx';
...
// Now do BottomPanel
const oldBottomStart = content.indexOf('export function BottomPanel({');
const oldBottomEnd = content.indexOf('/* ------------------------------------------------------------------ */', oldBottomStart) > -1 ? content.indexOf('/* ------------------------------------------------------------------ */', oldBottomStart) : content.length;
const oldBottom = content.slice(oldBottomStart, oldBottomEnd);

const newBottom = `export function BottomPanel({ region = "all", risk = "all" }: { region?: string; risk?: string }) {
...
fs.writeFileSync('packages/frontend/components/topology/topology-panels.tsx', content, 'utf8');
