...
const KIND_UI: Record<Identity["kind"], string> = {
  user: "User",
  role: "Role",
  "service-account": "Service Account",
};

const STATUS_UI: Record<Identity["status"], { label: string; dot: string }> = {
...
export default function IdentitiesPage() {
  ...
  const avgRisk = filtered.length ? Math.round(filtered.reduce((s, i) => s + i.risk, 0) / filtered.length) : 0;
              ...
              {filtered.map((id) => {
                ...
                return (
                  <tr key={id.id} className="border-b border-border/60 transition hover:bg-muted/50">
                    ...
                    <td className="px-4 py-3">
                      <ProviderMark provider={id.provider} size={26} />
                    </td>
